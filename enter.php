...
if (isset($_POST['name']) && isset($_POST['location'])) {
    ...
    $country_origin = sanitizeMySQL($conn, $_POST['country_origin']);
    $favorite_dish = sanitizeMySQL($conn, $_POST['favorite_dish']);

    // the prepared statement - note: 6 question marks represent
    // 6 variables we will send to database separately
    $query = "INSERT INTO restaurants (name, location, country_origin, favorite_dish)
    VALUES (?, ?, ?, ?)";
